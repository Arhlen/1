...
def get_pas_count(lines, level_age_30):
    total = [0, 0, 0]
    saved = [0, 0, 0]
    for line in lines:
        data = line.split(',')
        if data [6] != '':
            age = float(data[6])
            saved[int(age/30)] += int(data[1])
            total[int(age/30)] += 1
    return saved[level_age_30] / total[level_age_30] * 100, \
        (total[level_age_30] - saved[level_age_30]) / total[level_age_30] * 100

with open('data.csv') as file:
        text = file.readlines()

print(get_pas_count(text[1:], 0))
